...
(function (root, factory) {
    ...
    var Imgur = function (options) {
        ...
        this.dropzone2 = document.querySelectorAll('.dropzone2');
    ...
    Imgur.prototype = {
        ...
        createDragZone: function () {
            ...
            input = this.createEls('input', {type: 'file',required:' ', className: 'input',  accept: 'image/x-png, image/jpeg'});

            Array.prototype.forEach.call(this.info, function (zone) {
                zone.appendChild(p1);
                zone.appendChild(p2);
            }.bind(this));
            Array.prototype.forEach.call(this.dropzone2, function (zone) {
        ...
        loading: function () {
            ...
            div = this.createEls('div', {className: 'loading-modal2'});
            table = this.createEls('table', {className: 'loading-table2'});
                ...
                zone.addEventListener(event, function (e) {
                    if (e.target && e.target.nodeName === 'INPUT' && e.target.type === 'file') {
                        if (event === 'dragleave' || event === 'drop') {
                            e.target.parentNode.classList.remove('dropzone2-dragging');
                        } else {
                            e.target.parentNode.classList.add('dropzone2-dragging');
                        }
                    }
                }, false);
            });
